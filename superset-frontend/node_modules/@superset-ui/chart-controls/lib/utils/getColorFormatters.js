...
const getOpacity = (value, cutoffPoint, extremeValue) => {
  const MIN_OPACITY = 0.3;
  const MAX_OPACITY = 1;
  return extremeValue === cutoffPoint ? MAX_OPACITY : round(Math.abs((MAX_OPACITY - MIN_OPACITY) / (extremeValue - cutoffPoint) * (value - cutoffPoint)) + MIN_OPACITY, 2);
};
...
const getColorFunction = ({
  ...
  if (!_types.MULTIPLE_VALUE_COMPARATORS.includes(operator) && targetValue === undefined) {
    return () => undefined;
  }
